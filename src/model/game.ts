...
export function normalizeGame(game?: Partial<Game>): Game {
  ...
  game.board = game.board.map((card) => ({
    ...card,
    votesPerRound: normalizeArrayToCurrentRound<Votes>(
      () => [],
      game.round,
      card.votesPerRound
    ),
...
function normalizeArrayToCurrentRound<T>(
  def: () => T,
  round: Game["round"],
  arr?: T[]
): T[] {
...
export function roundsColor(game: Game): TeamColor {
  return game.round % 2 === 0
    ? game.startingColor
    : oppositeTeamColor(game.startingColor);
}
...
export function isCardsColorVisible(
  game: Game,
  player: Player,
  cardIndex: CardIndex
): boolean {
